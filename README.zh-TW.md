...
*   **全功能右鍵上下文選單 (原生檔案總管相容)**：
    *   **快速 Java 類別生成**：右鍵點選任一目錄或專案根目錄，即可透過 `新增 Java Class...`、`新增 Java Interface...`、`新增 Java Enum...`、`新增 Java Record...` 快速建立檔案，並自動根據路徑階層（如 `src/main/java`）精準注入 package 宣告！
    *   **標準檔案與資料夾操作**：支援 `新增檔案...`、`新增資料夾...`、`重新命名...`、以及安全的 `刪除`（移至垃圾桶）。
    *   **整合導航與剪貼簿**：一鍵 `在檔案總管/Finder中顯示`、`在整合終端機中開啟`、`在 VS Code 檔案總管中顯示`、`複製完整路徑` 與 `複製相對路徑`。
    *   **標題列捷徑**：在 Explorer 面板標題列上提供專屬的 `新增檔案` 與 `新增資料夾` 圖示按鈕。
...
### 5. Ignis Arc Obsidian Dark (焱虹黑曜石暗色主題)

一款內建於擴充套件中、與焱虹品牌完美呼應的 VS Code 精緻顏色主題：
*   **黑曜石溫潤底色**：採用高級感十足的暗灰炭黑（`#0C0E12`）作為主背景，比起一般的純黑或偏藍暗色主題更具視覺層次，並有效減緩長時間開發的雙眼疲勞。
*   **焱火主調高亮**：語言關鍵字、控制流以及修飾子採用明亮的**暖橘（#FA9E42）**與**火光紅（#E24A35）**，完美呈現「焱」的活力意象。
*   **虹光漸層語法**：類別與型態採用高雅的**極光紫（#B794F4）**，字串則採用具虹彩質感的**青藍色（#38B2AC）**，打造出極致流暢、高對比的程式碼閱讀體驗。

![Ignis Arc Obsidian Dark Theme](images/theme_mockup.png)

---

### 6. Ignis Arc Workbench (互動式所見即所得主題產生器)

無需妥協於固定的配色！擴充套件內建了一款精美且完全離線可用的 **WYSIWYG 互動式主題產生器**：
*   **即時模擬工作區**：右側調色盤拖曳時，左側完整的 VS Code 編輯器（包含檔案總管、標籤頁、Status Bar 與實體 Java 程式碼）會以微秒級速度即時呈現最新色彩！
*   **直覺式調色盤**：細分 *編輯器視窗*、*工作區 UI* 與 *程式碼語法標記*（關鍵字、控制流、類別、方法、字串、註解、註解元），調配隨心所欲。
*   **內建雙經典預設**：一鍵在經典的 `Obsidian Dark (黑曜石)` 與療癒的 `Pastel Dream (粉彩夢境)` 兩大主視覺間切換並在此基礎上調整。
*   **一鍵安全匯出**：即時生成符合 VS Code 標準的 JSON 內容，並支援「一鍵下載主題檔」與「複製 JSON 代碼到剪貼簿」。

欲自訂您專屬的焱虹 IDE 配色，只需在瀏覽器中直接開啟 [workbench/index.html](file:///home/luke/projects/java-lsp/ignis-arc-java-ide-extension/workbench/index.html) 即可！
