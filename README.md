...
A premium, high-performance VS Code extension pack designed to bring the deep **project navigation, dependency browsing, OOP reference metrics, decompilation, and bytecode performance tuning power** of heavyweight IDEs (IntelliJ IDEA / Eclipse) directly into VS Code, with **zero extra JVM process memory overhead** and **real-time codebase metrics analysis**.
...
*   **Active Theme Integration**: Uses file `resourceUri` mapping with a custom `jdt-class` scheme to automatically inherit your active **VS Code File Icon Theme** (e.g. *Material Icon Theme* or *vscode-icons*) without triggering any filesystem query overhead!
...
### 2. Specialized OOP Usages & Implementations Code Lenses
...
### 3. Ignis Arc McCabe Complexity Lenses & Refactoring Sidebar

A premium code-review sidebar panel and dynamic method lenses that scan your active Java workspace for complex methods, sorting them by cyclomatic complexity to help you instantly spot refactoring hot paths:
...
### 4. Ultimate Bytecode Performance Tuning Suite (Scroll Sync, Hover Explainer, & LVT)

Bring advanced low-level bytecode tuning directly into VS Code, with class structures, constant pools, variable tables, and instruction-level tracking synced side-by-side:
*   **🔁 Synchronized Scrolling & Highlighting**: Open your source code and bytecode side-by-side. Scrolling or selecting a line in the source code automatically scrolls to and highlights the matching JVM bytecode instructions via built-in `// IgnisSrcLine` markers!
*   **📖 JVM Instruction Hover Explainer**: Hover over any compiled JVM mnemonic (like `invokevirtual`, `aload_0`, `getfield`, `dup`, `new`) to view a premium popup showing its **operand stack transition diagram** and detailed runtime execution mechanics.
*   **📋 Local Variable Table (LVT) Explorer**: Prepend a clean ASCII table mapping local variable slots to their actual handwritten variable names and type descriptors at the beginning of each method bytecode body. No more guessing slot indices!
*   **⚡ Constant Pool Explorer**: Displays a beautifully formatted table at the very top of full class decompilations, mapping all class-level strings, constants, and method/field descriptors in detail.
*   **🧹 Lombok & Synthetic Method Filtering**: Toggle compiler-synthesized or Lombok methods out of both the method picker and decompilation views via a single configuration switch to clean up low-level noise.
...
| `ignis.java.bytecode.filterLombok` | `boolean` | `true` | Enable filtering out compiler-synthesized and Lombok methods from bytecode views. |
